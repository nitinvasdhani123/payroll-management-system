...
function Searchadmin({ name }) {
  ...
  const sendData = async (employeeId) => {
    try {
      const response = await axios.post('http://localhost:8000/api/search-employee', {
  ...
  };//autoload after delete

  //delete
  const deleteData = async (employeeId) => {
    try {
      let response2 = await axios.post('http://localhost:8000/api/delete-employee', {
        employeeId: employeeId,
      });
    ...
    <div id='main'>
      ...
      <div id='search'>
        ...
        <div className='searcharea'>
          <input type='search' onChange={(event) => setEmployeeId(event.target.value)} placeholder='Enter Admin Id' name='id'></input>
